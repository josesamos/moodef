...
Complementary, using the `moodef` package, we can define questionnaires from R. We have generalized 9 types of questions and simplified their definition, so that all the types are defined the same, the type is deduced from the definition. So, we define a question by calling a function or simply including a row in a data frame or a csv file. The result is an xml file that we import into [*Moodle*](https://moodle.org/). If necessary, some parameter not considered in the generalization can be defined or adjusted there.

Although not shown in the next example, in each question we can include images that are embedded in xml. We can set up the size of the images so that they are homogeneous when displayed in quizzes.
